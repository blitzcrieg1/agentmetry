# Splunk HEC — AgentAudit forwarder
...
BLACKBOX_AUDIT_SPLUNK_SOURCETYPE=agentaudit:json
...
| `BLACKBOX_AUDIT_SPLUNK_SOURCETYPE` | No | Default `agentaudit:json` |
...
2. Name: `agentaudit`
3. Source type: `agentaudit:json` (or accept automatic JSON)
4. Index: `main` (or dedicated `agentaudit`)
...
```spl
index=main sourcetype=agentaudit:json action_outcome=denied
```
