# EU AI Act — Deployer Checklist (BLACKBOX Operator)

**Role mapping:** BLACKBOX repo author = **provider** (tool). You, running it on client data = **deployer**.
...
| Step | Action | BLACKBOX hook |
|------|--------|---------------|
| 1 | Enable the orchestrator for all production skills | `scripts/blackbox.bat start` |
| 2 | Do not delete `apps/orchestrator/data/events.db` or `runs.jsonl` during an audit window | Immutable outbox + ledger |
| 3 | Export monthly evidence | `blackbox export --evidence --from … --to …` |
...
| 2 | Prefer local LLM for sensitive matters | `BLACKBOX_LLM_PROVIDER=ollama` |
...
| 4 | Review weekly stats | `blackbox stats --days 7` |
