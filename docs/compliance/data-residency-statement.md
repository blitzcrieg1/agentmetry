...
**Product:** BLACKBOX (Obsidian-Cortex Agentic OS)  
...
BLACKBOX processes business data **on infrastructure you control**. The Obsidian vault, run ledger, event outbox, and optional local LLM (Ollama) stay on your machine or your LAN. No multi-tenant cloud backend is required for core operation.
...
|----------|--------|-----------|
| **Ollama** | `BLACKBOX_LLM_PROVIDER=ollama` | Prompts/responses stay on localhost |
| **Gemini** | `BLACKBOX_LLM_PROVIDER=gemini` | Google API — review Google DPA / terms |
...
When using BLACKBOX on client PII:
...
5. Attach monthly `blackbox export --evidence` as technical annex.
